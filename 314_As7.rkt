...
;; change-identifier : symbol symbol λ-exp -> λ-exp --  helper function for alpha transformer,
...
;; bounded-identifier : λ-exp -> (listof symbol) --  find bounded identifiers in given λ expression.
...
;; generate-identifier : λ-exp -> symbol -- generate unpresented identifier.
...
;; alpha-transformer : λ-exp -> λ-exp -- apply alpha transformation on given λ-expression
;; (λ x M) --> (λ y [M:x=y])
...
;;-----------------------------------------------------------------------------------------------------------------------------------------------------
;; expt : number number -> number -- calculate exponentiation of given two number, first number base and second is power.
...
;; sqr : number -> number --  calculate square of given number.
...
;; get-op : symbol -> ((number number) -> number) -- obtain binary defined operation from operation definition table.
(define (get-op (sym : symbol)) : (number number -> number)
  (sym-op-op (assoc sym ops)))

;; assoc : symbol (listof pair) -> pair -- associate given symbol with operation defined in a listof pairs.
...
;; parseC : s-exp -> calC -- parseC given s-exp to abstract syntax calC
...
(test (parseC (number->s-exp 2))(calC-num 2))
(test (parseC (symbol->s-exp 'x))(calC-id 'x))
(test (parseC '(ifzero 5 4 9))(if-greater-than-zero (calC-num 5)(calC-num 4)(calC-num 9)))
(test (parseC '(ifzero 7 2 6))(if-greater-than-zero (calC-num 7)(calC-num 2)(calC-num 6)))
(test (parseC '(ifzero 1 3 5))(if-greater-than-zero (calC-num 1)(calC-num 3)(calC-num 5)))
(test (parseC '(ifzero 8 5 4))(if-greater-than-zero (calC-num 8)(calC-num 5)(calC-num 4)))
...
;; get-fundef : symbol (listof FunDefC) -> FunDefC -- find given symbol's(function name/identifier) function definition
...
;; Tests:
(test (lookup 'x EnvNameSpace) 2)
(test (lookup 'y EnvNameSpace) 8)
(test (lookup 'z EnvNameSpace) 4)
...
(define (interpC [e : calC] [env : Environment][fds : (listof FunDefC)]) : number
  (type-case calC e
    [calC-num (n) n]
    [calC-id (s) (lookup s env)]
...
(test (evalC '(sqr 5)) 25)
(test (evalC '(neg 4)) -4)
(test (evalC '(custom 5 7)) 17)
(test (evalC '(factorial 0)) 1)
(test (evalC '(factorial 1)) 1)
(test (evalC '(factorial 3)) 6)
(test (evalC '(factorial 6)) 720)
(test (evalC '(factorial 30)) 265252859812191058636308480000000)
...
(test (evalC '(fibonacci 15)) 610)
(test (evalC '(fibonacci 20)) 6765)
